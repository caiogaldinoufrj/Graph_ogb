...
def hinge_auc_loss(pos_out, neg_out, num_neg):
    ...
    neg_out = torch.reshape(neg_out, (-1, num_neg))
    # Trocado sum() por mean()
    return (torch.square(torch.clamp(1 - (pos_out - neg_out), min=0))).mean()

def weighted_auc_loss(pos_out, neg_out, num_neg, weight):
    weight = torch.reshape(weight, (-1, 1))
    pos_out = torch.reshape(pos_out, (-1, 1))
    neg_out = torch.reshape(neg_out, (-1, num_neg))
    # Trocado sum() por mean() - CUIDADO: Esta função penaliza acertos excessivos! Prefira a Hinge.
    return (weight*torch.square(1 - (pos_out - neg_out))).mean()
...
def weighted_hinge_auc_loss(pos_out, neg_out, num_neg, weight):
    ...
    pos_out = torch.reshape(pos_out, (-1, 1))
    neg_out = torch.reshape(neg_out, (-1, num_neg))
    # Trocado sum() por mean() - Esta é a função ideal para o ogbl-collab ponderado
    return (weight*torch.square(torch.clamp(weight - (pos_out - neg_out), min=0))).mean()
...
def adaptive_hinge_auc_loss(pos_out, neg_out, num_neg, weight):
    ...
    neg_out = torch.reshape(neg_out, (-1, num_neg))
    # Trocado sum() por mean()
    return (torch.square(torch.clamp(weight - (pos_out - neg_out), min=0))).mean()

def log_rank_loss(pos_out, neg_out, num_neg):
    pos_out = torch.reshape(pos_out, (-1, 1))
    neg_out = torch.reshape(neg_out, (-1, num_neg))
    # LogRank (BPR) já usava mean(), está perfeita.
    return -torch.log(torch.sigmoid(pos_out - neg_out) + 1e-15).mean()

def ce_loss(pos_out, neg_out):
    # Cross Entropy já usava mean(), está perfeita.
...
def info_nce_loss(pos_out, neg_out, num_neg):
    ...
    # InfoNCE já usava mean(), está perfeita.
